...
The work plan for AgileFlow spans the 12-week core development cycle (January 6 - March 27, 2026), during which the team built the Supabase backend, developed an AI collaboration engine, implemented multiple board views, and set up a testing infrastructure.

## 3.1. Work breakdown structure (WBS)

The AgileFlow WBS splits into two parallel streams: Software Engineering (technical implementation) and Management Engineering (process analysis, documentation, and validation).
...
|   +-- 1.1.2 Entity service layer (12 services: Board, Item, User, Sprint, etc.)
...
|   +-- 1.2.1 OpenRouter API integration (model cascade, cascading fallback)
...
|   +-- 1.2.4 Streaming response pipeline (progressive rendering in chat UI)
...
|   +-- 1.5.3 Accessibility audits (axe-core, WCAG 2.1 AA checks on login page)
...
The matrix below assigns each team member as Responsible (R) or Support (S) across all work packages.
...
AgileFlow used a parallel development model with explicit dependencies between components.
...
- Entity services (1.1.2) depend on schema design (1.1.1), because services need table definitions first
- AI tool framework (1.2.2) depends on entity services (1.1.2), because tools call CRUD operations
- Assignment algorithm (1.2.3) depends on entity services to query items and profiles
...
- Management Engineering documentation runs alongside technical implementation throughout the project
...
AgileFlow was built almost entirely on free-tier services and open-source libraries. Total spending over the 12-week period was approximately $10-15 USD (roughly 350-500 TRY at current exchange rates), all of it on OpenRouter API credits for AI model usage during development and testing.
...
The only real expense was the OpenRouter API. Supabase's free tier handles the database and authentication, while Vercel's hobby plan covers hosting and CDN delivery.
...
|---|---|---|---|---|
| R1 | AI API Rate Limiting (OpenRouter daily request limits: 1,000/day with credits) | Medium | Medium | Cascading model fallback across 4 models. Free-tier models (Llama, Gemini) as backup. With 10+ credits, 1,000 req/day is sufficient for team usage. |
| R2 | Supabase Free Tier Limits (500MB database, 5GB egress/month) | Low | Medium | Current usage is ~5MB, which is 1% of the limit. Monitor via Supabase dashboard. Upgrade path available at $25/mo Pro tier. |
...
| R4 | Data Integrity (data corruption from invalid inputs or schema inconsistencies) | Medium | High | Schema validation with Zod. RLS policy testing. Foreign key constraints with cascading deletes. Manual verification of all entity CRUD operations. |
