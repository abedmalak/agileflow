...
The AI sub-system adds intelligent collaboration features through the OpenRouter API, a unified gateway to multiple LLM providers. It covers a conversational assistant with tool-calling, an intelligent task assignment algorithm, streaming response delivery, persistent chat sessions, and analytics dashboards with AI-generated explanations.
...
LLMs have evolved from text-generation tools into function-calling agents that interact with external systems. The tool-calling paradigm works by having the model output structured JSON describing which function to call and with what parameters, letting the assistant take real actions rather than just describing them. OpenAI pioneered this with their function calling API; Anthropic, Google, and Meta have since adopted it as standard.

Using multiple models through an aggregator like OpenRouter addresses the reliability and cost problems that come with single-provider integrations. A cascade ordered by cost and capability keeps the application available even when premium models hit rate limits. Free-tier models (Llama 3.3 70B, Gemini 2.5 Flash) serve as fallbacks when GPT-4o-mini or Claude Haiku 4.5 are unavailable.

For task assignment, the team used a Multi-Criteria Decision Making (MCDM) approach inspired by the Analytic Hierarchy Process (AHP). The algorithm scores each candidate across three weighted dimensions (competency match, workload availability, and historical performance) and returns a normalized suitability score the system can explain to the user.
...
| Gemini 2.5 Flash (Google) | Free-tier fallback model |
| Claude Haiku 4.5 (Anthropic) | High-quality fallback model |
...
The AI system is a React Context (`AIProvider`) that wraps the application:

- **AIProvider** (`src/components/ai/AIProvider.jsx`): Manages AI state, including active session, message history, streaming state, model selection, and tool execution loop.
...
const MODEL_CASCADE = [
  ...
  'meta-llama/llama-3.3-70b-instruct:free',    // Fallback 1: free tier
  'google/gemini-2.5-flash',                   // Fallback 2: free tier
  'anthropic/claude-haiku-4.5',                // Fallback 3: high quality
];
```

The cascade is tried sequentially. If a model returns an error (rate limit, server error, invalid response), the system catches it and tries the next model. If all models fail, the user sees a friendly error message.
